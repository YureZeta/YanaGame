...
Este repositório tem uma implementação estática em HTML5, CSS3 e JavaScript para um jogo tipo "Ache Waly" com uma UI adaptativa para mouse e touch screen.
...
- `assets/Mapa.jpeg` — imagem de fundo principal da cena e mini mapa reduzido.
- `assets/mini-map.svg` — mini mapa opcional visual preservado para compatibilidade.

## Customização de assets

Para trocar a imagem do cenário e o mini-mapa, edite os assets na pasta `assets/` e mantenha o nome do arquivo em sincronismo com as referências do código.
...
A função `startGame()` inicia temporizador e muda o estado de execução. A função `handleSearch()` converte coordenadas de clique/touch em percentuais do tabuleiro e usa `findYanaAt()` para localizar a Yana mais próxima dentro da tolerância de 2,8% do mapa. Quando uma Yana é encontrada, `handleFound()` incrementa a contagem, atualiza a lista e emite a mensagem com `flashToast()`.
